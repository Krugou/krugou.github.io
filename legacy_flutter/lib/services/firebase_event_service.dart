...
class FirebaseEventService {
  ...
  CollectionReference get _userEventsCollection =>
      _firestore.collection('users').doc(_userId).collection('customEvents');
  ...
  Future<List<EventTemplate>> _loadCustomEvents() async {
    try {
      ...
      return snapshot.docs
          .map((doc) => EventTemplate.fromJson(doc.data()))
          .toList();
    } catch (e) {
  ...
  Future<void> uploadJsonEventsToFirebase() async {
    try {
      ...
      for (final entry in territoryEvents.entries) {
        territoryEventsData[entry.key.name] =
  ...
  Map<TerritoryType, List<EventTemplate>> _parseTerritoryEventsFromFirebase(
  ...
  List<EventTemplate> _parseMilestoneEventsFromFirebase(
      Map<String, dynamic> data) {
  ...
  Future<Map<TerritoryType, List<EventTemplate>>> _loadEventsFromJson() async {
    try {
      final territoryEventsJson =
          await _loadJsonAsset('assets/events/territory_events.json');
  ...
  Future<List<EventTemplate>> _loadMilestoneEventsFromJson() async {
    try {
      final milestoneEventsJson =
          await _loadJsonAsset('assets/events/milestone_events.json');
  ...
  Future<String> _loadJsonAsset(String path) async {
    // Use rootBundle to load JSON assets
