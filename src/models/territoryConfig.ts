...
export const getCapacity = (config: TerritoryConfig): number => {
  return config.threshold * config.capacityMultiplier * config.capacityBaseMultiplier;
};

export const getAvailableConfigs = (totalPopulation: number): TerritoryConfig[] => {
  return territoryConfigs.filter((config) => totalPopulation >= config.threshold);
};
...
export const getConfigById = (id: string): TerritoryConfig | null => {
  return territoryConfigs.find((config) => config.id === id) || null;
};

export const getConfigByType = (type: TerritoryType): TerritoryConfig | null => {
  return territoryConfigs.find((config) => config.type === type) || null;
};
