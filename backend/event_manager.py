...
class EventManagementWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.copy_button = QPushButton("Copy Event")
    ...
    def copy_event(self):
        """Copy selected event to create a new one"""
        current_row = self.events_table.currentRow()
        if current_row < 0:
            QMessageBox.warning(self, "Warning", "Please select an event to copy")
            return

        # Get the selected event
        event = self.events[current_row].copy()

        # Modify the ID to make it unique
        original_id = event.get('id', '')
        event['id'] = f"{original_id}_copy_{datetime.now().strftime('%Y%m%d_%H%M%S')}"

        # Open edit dialog with the copied event data
        dialog = EventEditDialog(event, parent=self)
        dialog.setWindowTitle("Copy Event - Edit Details")
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if self.firebase_manager.add_event(event_data, territory_type):
                QMessageBox.information(self, "Success", "Event copied successfully")
                self.refresh_events()
            else:
                QMessageBox.warning(self, "Error", "Failed to copy event")
